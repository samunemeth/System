	...
	vim.ui.input({ prompt = "Calculator: " }, function(input)
		eval_math(input, function(result)
			vim.api.nvim_feedkeys(
				vim.api.nvim_replace_termcodes("i" .. result .. "<esc>", true, false, true),
				"i",
				true
			)
		end)
...
vim.keymap.set("n", "ma", function()
	vim.ui.input({ prompt = "Calculator: " }, function(input)
		eval_math(input, function(result)
			vim.api.nvim_feedkeys(
				vim.api.nvim_replace_termcodes("a" .. result .. "<esc>", true, false, true),
				"i",
				true
			)
		end)
...
vim.keymap.set("v", "mm", function()
	...
	eval_math(input[1], function(result)
		vim.api.nvim_feedkeys(
			vim.api.nvim_replace_termcodes("di" .. result .. post_keys .. "<esc>", true, false, true),
			"i",
			true
		)
	end)
